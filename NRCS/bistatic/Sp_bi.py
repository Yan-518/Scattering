...
def pol_vec_sp(theta_i, theta_s, eq_azi, inc_polar):
    """
    :param theta_i:
    :param eq_azi:
    :param polarization:
    ...
    (rot_ang_1, rot_ang_2, rot_ang_tot, Ps1, Ps2, Ps_tot) = elfouhaily(poli, 0, np.degrees(theta_i),
                                                                       np.degrees(bist_ang_az), np.degrees(theta_s))
...
def Sp_bi(kr, theta_i, theta_s, theta_eq, eq_azi, u_10, fetch, spec_name, inc_polar):
    """
    :param k:
    :param kr:
    :param theta_i:
