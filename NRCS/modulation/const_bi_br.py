...
from NRCS.modulation.const_brmo import const_ni2_func
from NRCS.modulation.Br_bi_modulation import B_int_single

def const_eq_br_mo(k, K, kr, theta_eq, bist_ang_az, eq_azi, wind_dir, u_10, fetch, div, polarization):
    """
    ...
    for ii in np.arange(ni2.shape[0]):
        for jj in np.arange(ni2.shape[1]):
            ...
            kkbr = np.sort(kbr[:,0])
            T = Trans_func(kkbr, K[ii, jj], u_10, fetch, azimuth, div[ii, jj])[np.argsort(kbr[:,0])].reshape(nnk,1)
            Skb = B_int_single(kkbr.reshape(nnk, 1), u_10, fetch, eq_azi[jj])[np.argsort(kbr[:,0]), :] * (1+abs(T)) / kbr.reshape(nnk,1) ** 4
            Skb_pi = B_int_single(kkbr.reshape(nnk, 1), u_10, fetch, np.pi + eq_azi[jj])[np.argsort(kbr[:,0]), :] * (1+abs(T)) / kbr.reshape(nnk, 1) ** 4
...
def const_bi_br(k, K, kr, theta_i, theta_s, theta_eq, bist_ang_az, eq_azi, u_10, wind_dir, fetch, div, polarization, inc_polar, sat):
    M = pol_vec_br(theta_i, theta_s, bist_ang_az, eq_azi, inc_polar, sat)
    Br = const_eq_br_mo(k, K, kr, theta_eq, bist_ang_az, eq_azi, wind_dir, u_10, fetch, div, polarization)
    return M * Br