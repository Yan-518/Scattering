...
def eq_wb_mo(kr, K, theta_eq, eq_azi, u_10, fetch, div):
    ...
    for ii in np.arange(div.shape[0]):
        for jj in np.arange(div.shape[1]):
            ...
            T = Trans_func(KK, K[ii, jj], u_10[ii, jj], fetch, phi1, div[ii, jj])
            ...
            Awb = np.trapz(np.cos(phi1 - eq_azi[jj]) * lamda, phi1, axis=1) / lamda_k
...
def Wb_bi_modulation(kr, K, theta_i, theta_s, theta_eq, bist_ang_az, eq_azi, u_10, fetch, div, polarization, inc_polar, re_polar, sat):
    ...
    WB, q = eq_wb_mo(kr, K, theta_eq, eq_azi, u_10, fetch, div)
    if polarization == 'VV':
        if re_polar == 'Bragg':
