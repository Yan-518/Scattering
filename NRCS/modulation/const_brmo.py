...
def const_brmo(k, K, kr, theta, azimuth, u_10, fetch, wind_dir, ind_pi, div, polarization):
    """
    :param k:
    :param kr:
    :param theta:
    ...
    for ii in np.arange(ni2.shape[0]):
        for jj in np.arange(ni2.shape[1]):
            ...
            kkbr = np.sort(kbr)
            T = Trans_func(kkbr[:, 0], K[ii, jj], u_10, fetch, azimuth, div[ii, jj])[np.argsort(kbr[:,0])].reshape(nnk,1)
            spec_Skk = kudryavtsev05(kkbr.reshape(nnk, 1), u_10, fetch, azimuth)[np.argsort(kbr[:,0]), :] * (1+abs(T)) / kbr.reshape(nnk, 1) ** 4
